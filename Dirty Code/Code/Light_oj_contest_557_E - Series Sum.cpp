...
main()
{
    ...
    {
        scanf("%lld",&n);
        LLI sum=((6%MOD)*(BIGMOD(n,(LLI)5,(LLI)MOD)%MOD))%MOD;
        //cout<<sum<<endl;
        LLI p=((15%MOD)*(BIGMOD(n,(LLI)4,MOD))%MOD)%MOD;
        //cout<<p<<endl;
        sum=((sum%MOD)+(p%MOD))%MOD;
        p=((10%MOD)*(BIGMOD(n,(LLI)3,MOD))%MOD);
        sum=((sum%MOD)+(p%MOD))%MOD;
        p=BIGMOD(n,(LLI)1,MOD);
        sum=(((sum%MOD)-(p%MOD)+MOD))%MOD;
        sum=((sum%MOD)*(modInverse((LLI)30,MOD)%MOD))%MOD;
        printf("Case %d: %lld\n",t,sum);
    }
}
